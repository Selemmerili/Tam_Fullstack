...
echo (exemple :C:\Users\%USERNAME%\Desktop\dossier)
set /p cheminfichier2=Veuillez entrer le chemin exact vers le dossier du back:
:blank5
if "%cheminfichier2%" == "" (
    echo Entrez quelque chose !
    set /p cheminfichier2=Veuillez entrer le chemin exact vers le dossier du back:
...
set /p destinationfichier=Veuillez entrer la destination du fichier sur la vm: 
:blank4
...
scp -r %cheminfichier2% %LOGIN%@%ip%:%destinationfichier%

echo Le fichier à bien été envoyé !
pause
Echo Lancement du script sur la vm :

@REM set /p nmfich1=Veuillez entrer le nom du fichier bash à lancer: 
@REM echo exemple:%cheminfichier%
@REM :nmfich
@REM if "%nmfich1%" == "" (
@REM     echo Entrez quelque chose !
@REM     set /p nmfich=Veuillez entrer le nom du fichier bash à lancer:
@REM     goto :nmfich
@REM )
@REM pause
...
ssh -t %LOGIN%@%ip% "sudo apt install dos2unix"
ssh -t %LOGIN%@%ip% "dos2unix post-install.sh"
ssh %LOGIN%@%ip% "chmod 777 ./post-install.sh"
ssh -t %LOGIN%@%ip% "sudo -S ./post-install.sh"
